...
def main():
    ...
    for parents in num_random_parents: # run a bunch of different parent combindations
        ...
        plot_mean_and_bootstrapped_ci_over_time(input_data = experiment_results, name = data_names, title=f'combination methods', x_label = "Generation", y_label = "Fitness", y_limit = [0,bit_string_length], plot_bootstrap = True)
        plot_mean_and_bootstrapped_ci_over_time(input_data = diversity_results, name = data_names, title=f'combination methods', x_label = "Generation", y_label = "Diversity", plot_bootstrap = True)
